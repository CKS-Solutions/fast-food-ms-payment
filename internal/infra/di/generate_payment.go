...
func NewGeneratePaymentContainer(region infra_aws.AwsRegion, stage infra_aws.AwsStage) *GeneratePaymentContainer {
	...
	paymentTopic := driven_sns.NewPaymentSNS(snsClient)

	return &GeneratePaymentContainer{
		GeneratePaymentUseCase: usecases.NewGeneratePaymentUseCase(paymentRepo, generateTokenMP, generateQRCodeMP, paymentTopic),
	}
}
